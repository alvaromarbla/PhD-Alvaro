...
phi = @(alpha, eps_eng) 0;%alpha + eps_eng;
...
P = @(V,n) CP_f(V,n,0,0)*N_eng*rho*n^3*D^5;
...
P_f = @(X) CP_f(X(1),X(2),X(3),-X(3))*N_eng*rho*X(2)^3*D^5;
...
x0_f = [20.225109 50.214554 0.046444 0.089094147 0.73578832 0];
...
vect_cc_xf = [2,4,8,10,15,xf_max_fmin,25,30,40,50,60];
...
%vect_cc_xf = linspace(min(min(xfmat)),max(max(xfmat)),N_contour_lines);
 ...
 hFIG1 = figure(1)
 fname = "cruise_plane_mode";

[xf_c,h_xfmat_c] = contourf(nlin,Vlin,xfmat,vect_cc_xf');
       clabel(xf_c,h_xfmat_c,"Interpreter","latex",'FontSize',12)
       colormap(flipud(colormap('gray')))
         grid on 
         title(strcat(['Range [km] vs. V [m/s] \& Engine rev. [rps]. Max Range is ', num2str(xf_max_fmin), ' km']),"Interpreter","latex")
         ...
         legend('$x_f$ (V,n)','T = D constrain. Full Model','Max RPS','Optimal solution','Location','northwest')
...
c  = [+sqrt(2*W/(rho*S_ref*CL_max_w1_CR_ope))-X(1);-rps_max+X(2);T(X(1),X(2),X(3),-X(3))-Tmax;X(3)-pi/2;-X(4);]; 

% T = D condition
ceq = [ T(X(1),X(2),X(3),-X(3)).*cos(phi(X(3),-X(3)))- 0.5*rho*X(1)^2*S_ref*X(4); 
    T(X(1),X(2),X(3),-X(3)).*sin(phi(X(3),-X(3)))+0.5*rho*X(1).^2*S_ref*X(5)-W;
    CL(X(3))-X(5);
    CD(X(3))-X(4)];
end