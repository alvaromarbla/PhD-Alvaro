...
CL_max_w1_ope = R_data.Performance.CL_max_w1_ope;
...
for j = 1:length(D_prop_vect)
    ...
    i = i+1;
    dist = R_data.Performance.Range;    %%% Distancia de la misión
 
    %% Velocidad inicial en m/s
    Vh(1) = R_data.Performance.V_min;
    ...
    while paro == 0
        ...
        J_solve = @(n_solve) Vh(cont)/(n_solve*D_prop);
...
%        
     ...
     %Lift coefficient with the new calculated mass
         CL_variation_mass(cont)=(MTOW_variation_mass(cont)*g)/(q_inf*S_ref);
        ...
        if  CL_variation_mass(cont) > CL_max_w1_ope %we have to include a condition for max CL 
            %(but without exiting the loop)
            Ti(cont)=0;
            Ti_eng(cont)=0;
            Pi(cont)=0;
            Pi_eng(cont)=0;
            ...
            Energy_kJ(cont)=0;
            Battery_mass(cont)=0;
            MTOW_variation_mass(cont)=0;
            CL_variation_mass(cont)=0;
            Qi_eng(cont)=0;
            delta_Tinter(cont)=0;
            Propulsive_mass(cont)=0;
            eta_total(cont)=0;
            RPM(cont)=0;
            nps_h=0;
        end
...
Y = Vh(1:contmax-1); % Horizontal Speed
...
%         Fig = Fig + 1;
        ...
        figure(Fig)
        [C_Pe_c,h_Pe_c] = contourf(X,Y,Z_Pe,'LevelStep', 0.8341);%vect_cc_Pe');
...
%         end
        ...
        figure(Fig)
        [C_delta_c,h_delta_c] = contourf(X,Y,Z_delta,'LevelStep', 0.035);%vect_cc_delta');
...
%         Fig = Fig + 1;
        ...
        figure(Fig)
        [C_Energy_c,h_Energy_c] = contour(X,Y,Z_Energy_kJ,vect_cc_Energy_kJ');
        clabel(C_Energy_c,h_Energy_c)
        grid on
        Title7 = strcat('Total Energy (kJ) vs. D_p_r_o_p & Horizontal Speed');
        title(Title7)
        xlabel('D_p_r_o_p (in)')
        ylabel('V_h (m/s)')
%         if SAVE_FIGS==1
...
%         
        figure(Fig)
        [C_MTOW_mass_c,h_MTOW_mass_c] = contourf(X,Y,Z_MTOW_mass,'LevelStep', 0.63);%vect_cc_MTOW_variation_mass');
        clabel(C_MTOW_mass_c,h_MTOW_mass_c)
        colormap(flipud(colormap('gray')))  %%%%%%%%%%%%%%%%%%%
        grid on
        Title10 = strcat('MTOW  (kg) vs. D_p_r_o_p & Horizontal Speed');
        title(Title10)
        xlabel('D_p_r_o_p (in)')
        ylabel('V_h (m/s)')
        sidebar = colorbar;  %%%%%%%%%%%%%%%%%%%
