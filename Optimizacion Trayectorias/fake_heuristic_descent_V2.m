...
for model = 1
   ...
   P = @(V,n) CP(V,n)*N_eng*rho*n^3*D^5;
    ...
    Dr  = 0.5*rho*V^2*S*CD(alpha_V);
Fun = @(n) T(0,n) - W + Dr; % T=W3
...
x_vec = zeros(1,Tlin);
h_vec = linspace(50,0,Tlin);
V_vec = V*ones(1,Tlin);
n_vec = n_sol(2,1)*ones(1,Tlin);
gamma_vec = -pi/2*ones(1,Tlin);
alpha_vec = -pi/2*ones(1,Tlin);
epsilon_vec = pi/2*ones(1,Tlin);
phi_vec = NaN(1,Tlin);
L_vec = NaN(1,Tlin);
T_vec = T_sol(2)*ones(1,Tlin);
D_vec = Dr*ones(1,Tlin);
...
Mission_11 = [t_vec; x_vec ; h_vec; V_vec; n_vec;gamma_vec; alpha_vec ; epsilon_vec; phi_vec; L_vec; T_vec; D_vec; P_vec; E_vec; E_vec_perc]';
save Mission_11.mat Mission_11
