...
n_opt = fminbnd(@(n) climb_calculator_E(n,eps_eng_val,alpha_cruise,flag_plot), 50, 65)
%n_opt = 50;
%% For plot purposes

flag_plot = 1;

climb_calculator_E(n_opt,eps_eng_val,alpha_cruise,flag_plot);

%% Solve system for rps

function J_obj = climb_calculator_E(n,eps_eng_val,alpha_cruise,flag_plot)
global D
global N_eng
global rho
global delta0
...
[t,y] = ode45(@(t,y)fun(t,y,n,eps_eng_val,alpha_cruise),tspan,y0,optionsolver);
...
CP = @(y,n,alpha, eps_eng)  a1_P + b1_P*J(y) + c1_P*J(y).^2 + d1_P*J(y).^3 + e1_P*J(y).^4 + ... 
    phi(alpha, eps_eng).*(b2_P*J(y) + c2_P*J(y).^2 + d2_P*J(y).^3) +  phi(alpha, eps_eng).^2.*(b3_P*J(y) + c3_P*J(y).^2)...
    +  phi(alpha, eps_eng).^3.*b4_P*J(y);
...
P_vec     =  CP(V_vec,n,eps_eng_val,alpha_cruise).*rho*N_eng*n.^3*D^5;
...
if flag_plot ==1
    ...
    Title1 = strcat(['Trajectory Angle \gamma vs time. Engine revolutions = ', num2str(n), 'rps, \phi =', num2str((eps_eng_val+alpha_cruise)*180/pi)],'deg');
    title(Title1)
    ...
    Title1 = strcat(['Horizontal distance vs time. Engine revolutions = ', num2str(n),'rps, \phi =', num2str((eps_eng_val+alpha_cruise)*180/pi)],'deg');
    title(Title1)
    ...
    Title1 = strcat(['Vertical distance vs time. Engine revolutions = ', num2str(n), 'rps, \phi =', num2str((eps_eng_val+alpha_cruise)*180/pi)],'deg');
    title(Title1)
    ...
    Title1 = strcat(['Flight speed vs time. Engine revolutions = ', num2str(n), 'rps, \phi =', num2str((eps_eng_val+alpha_cruise)*180/pi)],'deg');
    title(Title1)
    ...
    Title1 = strcat(['Energy consumption vs time. Engine revolutions = ', num2str(n), 'rps, \phi =', num2str((eps_eng_val+alpha_cruise)*180/pi)],'deg');
    title(Title1)
    ...
    Title1 = strcat(['Power consumption vs time. Engine revolutions = ', num2str(n), 'rps, \phi =', num2str((eps_eng_val+alpha_cruise)*180/pi)],'deg');
    title(Title1)
    ...
    ylabel('Vertical distance [m]')
    Title1 = strcat(['Full Trajectory. Engine revolutions = ', num2str(n), 'rps, \phi =', num2str((eps_eng_val+alpha_cruise)*180/pi)],'deg');
    title(Title1)
...
function dy = fun(t,y,n,eps_eng_val,alpha_cruise)
...
CT = @(y,n,alpha,eps_eng)  a1_T + b1_T*J(y) + c1_T*J(y).^2 + d1_T*J(y).^3 + e1_T*J(y).^4 + ... 
    phi(alpha,eps_eng).*(b2_T*J(y) + c2_T*J(y).^2 + d2_T*J(y).^3) +  phi(alpha,eps_eng).^2.*(b3_T*J(y) + c3_T*J(y).^2)...
    +  phi(alpha,eps_eng).^3.*b4_T*J(y);
...
T   = @(y,n,alpha,eps_eng) CT(y,n,alpha,eps_eng)*N_eng*rho*n^2*D^4;
...
P = @(y,n,alpha,eps_eng) CP(y,n,alpha,eps_eng)*N_eng*rho*n^3*D^5;
...
dy(3) = (T(y,n,alpha_cruise,eps_eng_val)*cos(phi(alpha_cruise,eps_eng_val)) - Dr(y))/mTOW -g*sin(y(4)); % Long forces equation
dy(4) = L(y)/(mTOW*y(3)) + T(y,n,alpha_cruise,eps_eng_val)*sin(phi(alpha_cruise,eps_eng_val))/(mTOW*y(3)) - g*cos(y(4))/y(3); % Transversal forces equation
dy(5) = P(y,n,alpha_cruise,eps_eng_val)/((1-tau)*eta_m);
end
