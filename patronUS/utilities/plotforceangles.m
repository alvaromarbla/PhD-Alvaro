function plotforceangles(results)
    % Extract parameters
    V = results.V_opt;
    gamma = results.gamma_opt;  % Flight path angle (airflow direction)
    alpha = results.alpha_opt;  % Angle of attack (vehicle vs airflow)
    epsilon = results.epsilon_opt; % Thrust vector angle (engine vs vehicle)
